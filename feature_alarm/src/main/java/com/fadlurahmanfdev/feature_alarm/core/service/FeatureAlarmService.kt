...
abstract class FeatureAlarmService : Service() {
    ...
    companion object {
        ...
        fun <T : FeatureAlarmService> startPlayingAlarm(
            ...
            val intent = Intent(context, clazz).apply {
                action = FeatureAlarmConstant.ACTION_PLAY_ALARM
                putExtra(FeatureAlarmConstant.PARAM_NOTIFICATION_ID, notificationId)
                if (bundle != null) {
                    putExtras(bundle)
                }
            }
        ...
        fun <T : FeatureAlarmService> stopPlayingAlarm(
            ...
            val intent = Intent(context, clazz).apply {
                action = FeatureAlarmConstant.ACTION_STOP_ALARM
            }
            ContextCompat.startForegroundService(context, intent)
        }
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            FeatureAlarmConstant.ACTION_PLAY_ALARM -> {
                val notificationId = intent.getIntExtra(FeatureAlarmConstant.PARAM_NOTIFICATION_ID, -1)
            ...
            FeatureAlarmConstant.ACTION_STOP_ALARM -> {
